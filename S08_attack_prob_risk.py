...
def pca_reduction(match_data_x, target_data_x, n_comp):
    ...
    # 通过 当前中心 得到转换矩阵
    new_target_data_x = pca_model.fit_transform(target_data_x)
    new_match_data_x = pca_model.transform(match_data_x)
...
def pca_reduction_with_similar_weight(match_data_x, target_data_x, similar_weight, n_comp):
    ...
    pca_model = PCA(n_components=n_comp, random_state=2022)
    # 转换需要 * 相似性度量
    new_target_data_x = pca_model.fit_transform(target_data_x * similar_weight)
    new_match_data_x = pca_model.transform(match_data_x * similar_weight)
    ...
    logger.info("成功对目标患者进行恢复数据:  target_data: {}".format(rec_tar_x_df.shape))
...
def do_cal_auc_info(target_sens_true, target_sens_predict):
    """
    计算损失
    :param target_sens_true: 目标敏感真实值
    :param target_sens_predict: 目标敏感预测值
    ...
    auc_list = []
    for col in cols:
        auc_list.append(roc_auc_score(target_sens_true[col], target_sens_predict[col]))
...
def buildLogisticRegressionModel(cur_col, match_hos_train_data_x, match_hos_train_data_y, target_hos_test_data_x):
    """
    建立回归模型
    :param cur_col:
    :param match_hos_train_data_x:
    ...
    model.fit(match_hos_train_data_x, match_hos_train_data_y)
    target_hos_test_data_y_predict = model.predict_proba(target_hos_test_data_x)[0][1]
...
def do_build_model(match_hos_train_data_x, match_hos_train_data_ys, target_hos_test_data_x, sens_cols):
    ...
    for cur_col in sens_cols:
        match_hos_train_data_y = match_hos_train_data_ys[cur_col]
        if len(match_hos_train_data_y.unique()) == 1:
            result_df.loc[target_hos_test_data_x.index, cur_col] = match_hos_train_data_y.unique()[0]
            continue
        # result_df[cur_col] = buildRegressionModel(cur_col, match_hos_train_data_x, match_hos_train_data_y, target_hos_test_data_x)
        result_df.loc[target_hos_test_data_x.index, cur_col] = buildLogisticRegressionModel(cur_col, match_hos_train_data_x, match_hos_train_data_y, target_hos_test_data_x)

    if result_df.isna().sum().sum() > 0:
        logger.error("结果出现异常...")
        raise Exception("建模出现错位...")

    logger.info(f"target_index:{target_hos_test_data_x.index[0]} 预测完成...")
...
def get_similar_rank_reduction(len_split, target_pre_data_select, pca_match_data_x):
    """
    选择前10%的样本，并且根据相似得到样本权重
    :param pca_match_data_x:
    :param len_split:
    :param target_pre_data_select:
    :return:
    """
    try:
        similar_rank = pd.DataFrame(index=pca_match_data_x.index)
        similar_rank['distance'] = abs(pca_match_data_x - target_pre_data_select.values).sum(axis=1)
        similar_rank.sort_values('distance', inplace=True)
        patient_ids = similar_rank.index[:len_split].values
        sample_ki = similar_rank.iloc[:len_split, 0].values
        sample_ki = [(sample_ki[0] + m_sample_weight) / (val + m_sample_weight) for val in sample_ki]
...
def main_run_with_psm(from_hos_id, to_hos_id, n_comp):
    ...
    _, target_data_x, _, target_data_y = get_fs_each_hos_data_X_y(from_hos_id)
    match_data_x, _, match_data_y, _ = get_fs_each_hos_data_X_y(to_hos_id)
    match_init_similar_weight = get_init_similar_weight(from_hos_id)
    len_split = int(match_data_x.shape[0] * 0.1)
    columns_list = match_data_x.columns.to_list()
    ...
    logger.warning(f"开始多线程计算目标患者损失值... pool:{pool_nums}, nums:{len(target_index_list)}")
    with ThreadPoolExecutor(max_workers=pool_nums) as executor:
        thread_list = []

        for target_index in target_index_list:
            # cur_target_x = recover_target_data_x.loc[[target_index], :]
            pca_cur_target_x = pca_target_data_x.loc[[target_index], :]
            # 获取相似样本(降维相似性度量匹配）
            patient_ids, _ = get_similar_rank_reduction(
                len_split=len_split,
                target_pre_data_select=pca_cur_target_x,
                pca_match_data_x=pca_match_data_x
            )
            ...
            fit_test_x = target_hos_test_data_x.loc[[target_index], :]
            # 3.3. 预测对应的敏感特征信息并生成新的矩阵
            thread = executor.submit(do_build_model, fit_train_x, fit_train_y, fit_test_x, sens_cols)
            thread_list.append(thread)

        wait(thread_list, return_when=ALL_COMPLETED)
        for thread in thread_list:
            target_sens_predict = pd.concat([target_sens_predict, thread.result()], axis=0)

    # 4. 通过损失函数计算对应的损失信息
    target_sens_true = target_data_x[sens_cols]  # 敏感真实值
    target_sens_predict = target_sens_predict.loc[target_index_list, :]  # 调整索引顺序
    # loss_value = do_cal_loss_info(target_sens_true, target_sens_predict)
    loss_value = do_cal_auc_info(target_sens_true, target_sens_predict)

    logger.info("[使用相似性度量] - comp:{} - 计算得知当前AUC均值为: {}".format(comp, loss_value))
...
def main_run_with_no_psm(from_hos_id, to_hos_id, n_comp):
    ...
    logger.info("[不使用相似性度量] - comp:{} - 计算得知当前AUC均值为: {}".format(comp, loss_value))
...
if __name__ == '__main__':
    ...
    all_res_df.to_csv(os.path.join(save_path, f"S08_{from_hospital_id}_{to_hospital_id}_attack_loss_with_LR.csv"))

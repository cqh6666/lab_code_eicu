...
def fairness_with_view_only(score_used,subgroup_used,view_id_used):
    ...
    subgroup_score_used_fs = subgroup_score_used.loc[:,view_start[view_id_used]:view_end[view_id_used]].copy()
    selected_column_names = subgroup_score_used_fs.columns.tolist()
    subgroup_score_used_fs['predict_score'] = subgroup_score_used_fs.sum(axis=1)
    ...
    remain_score_used_fs = remain_score_used.loc[:,view_start[view_id_used]:view_end[view_id_used]].copy()
    remain_score_used_fs['predict_score'] = remain_score_used_fs.sum(axis=1)
...
for view_id in range(len(view_name)):
    ...
    if fairness_target == 'subgroups':
        ...
        view_result_record.loc[view_name[view_id],'AKI_score_global_view_CV':'AKI_score_person_noview_CV'] = np.array(score_std.values) / np.array(score_mean.values)
        
        aurcc_std = view_aurcc_result.std(ddof=0)
        aurcc_mean = view_aurcc_result.mean()
        view_result_record.loc[view_name[view_id],'AURCC_global_view_CV':'AURCC_person_noview_CV'] = np.array(aurcc_std) / np.array(aurcc_mean.values)

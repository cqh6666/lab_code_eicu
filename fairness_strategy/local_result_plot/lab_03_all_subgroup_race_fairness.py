...
my_data_df = pd.read_csv(f"/home/chenqinhai/code_eicu/my_lab/fairness_strategy/local_result/lab_03_subgroup_race_{race}_top20_fairness_to_plot.csv", index_col=0)
...
axesSub.set_xlabel("从每个亚组选取 Top-K%")
...
select_cond = (my_data_df["threshold_type"] == f"根据各亚组{race_name}比例阈值") & (my_data_df["score_type"] == "TPR")
plot_df = my_data_df[select_cond]
axesSub2 = sns.barplot(data=plot_df,errorbar='se', x="threshold", y="score", hue="建模策略", capsize=.06, errwidth=1.2, ax=ax[1])
# g.set_axis_labels("从全部样本中选取 Top-K", "召回率(TPR)")
axesSub2.set_xlabel(f"根据每个亚组的{race_name}群体选取的 Top-K%")
...
# save fig
plt.savefig(f'lab_03_{race}_result.jpg', dpi=1500, bbox_inches='tight')
plt.show()

...
all_save_path = f"/home/chenqinhai/code_eicu/my_lab/result/S05/"
hos_path = "/home/chenqinhai/code_eicu/my_lab/result/S05/{}"
for hid in hos_id_list:

    for boost in local_boost:
        for select in select_rate:
            try:
                # S05_lr_test_tra0_boost100_comp100_v1.csv
                res_tra = pd.read_csv(os.path.join(hos_path.format(hid), f"S05_{model}_test_tra1_boost{boost}_comp{select}_v{version}.csv"))
                res_no_tra = pd.read_csv(os.path.join(hos_path.format(hid), f"S05_{model}_test_tra0_boost{boost}_comp{select}_v{version}.csv"))

...
from S04_top20_subgroup_data import get_top20_drg_cols, get_top20_drg_data

warnings.simplefilter(action='ignore', category=FutureWarning)

# 获取DRG列表
# disease_list=pd.read_csv('/home/liukang/Doc/disease_top_20.csv')
disease_list = get_top20_drg_cols()
Label = 'aki_label'

# 获取对应种族
# race, race_id = 'black', 'race_African American'
race, race_id = 'white', 'race_Caucasian'
...
result_name['global'] = 'score_y_global'
result_name['personal'] = 'score_y_person'
# 结果记录
...
# top20总共有7209个AKI患者/16178个患者
threshold_used = [1800, 3600, 7200, 'self_10%', 'self_20%', 'self_40%', 'self_subgroup_aki_25%', 'self_subgroup_aki_50%', 'self_subgroup_aki_100%']
model_list = ['global', 'personal']

AKI_select_record = pd.DataFrame(index=threshold_used, columns=model_list)
nonAKI_select_record = pd.DataFrame(index=threshold_used, columns=model_list)
...
for disease_id_name in disease_list:
    # 获取对应亚组
    subgroup_feature_true = test_total.loc[:, disease_id_name] == 1
    subgroup_data_select = test_total.loc[subgroup_feature_true]  # 对应的subgroup
    subgroup_data_select = subgroup_data_select.reset_index(drop=True)

    # 字典方式保存所有亚族数据
    subgroup_all_race_result[disease_id_name] = subgroup_data_select
    subgroup_all_race_result_total = pd.concat([subgroup_all_race_result_total, subgroup_data_select])

    # 获取对应亚组的种族
    subgroup_race_true = subgroup_data_select.loc[:, race_id] == 1
    subgroup_race_select_result = subgroup_data_select.loc[subgroup_race_true]
    subgroup_race_select_result = subgroup_race_select_result.reset_index(drop=True)

    # 字典方式保存所有亚组的种族数据，并保存为一个新的dataFrame
    subgroup_result[disease_id_name] = subgroup_race_select_result
    subgroup_result_total = pd.concat([subgroup_result_total, subgroup_race_select_result], axis=0)
...
for model in model_list:
    subgroup_all_race_result_total.sort_values(result_name[model], ascending=False, inplace=True)
    subgroup_all_race_result_total.reset_index(drop=True, inplace=True)
    threshold_record.loc['{}_{}'.format(model, threshold_used[0]), 'threshold'] = subgroup_all_race_result_total.loc[
        threshold_used[0] - 1, result_name[model]]
    threshold_record.loc['{}_{}'.format(model, threshold_used[1]), 'threshold'] = subgroup_all_race_result_total.loc[
        threshold_used[1] - 1, result_name[model]]
    threshold_record.loc['{}_{}'.format(model, threshold_used[2]), 'threshold'] = subgroup_all_race_result_total.loc[
        threshold_used[2] - 1, result_name[model]]
...
for disease_id_name in disease_list:
    ...
    for model in model_list:
        ...
        for threshold_id in threshold_used:
            ...
            elif threshold_id == 'self_40%':
                threshold_value = data_all_race_subgroup.loc[int(data_all_race_subgroup.shape[0] * 0.4),result_name[model]]

            elif threshold_id == 'self_subgroup_aki_25%':
                threshold_value = data_all_race_subgroup.loc[int(data_all_race_subgroup_AKI.shape[0] * 0.25), result_name[model]]
...
threshold_type_dict = {
    "Top-K from all subgroups": threshold_used[:3],
    "Top-k% from all subgroups": threshold_used[3:6],
    "Top-K% incidence rate from each subgroup": threshold_used[6:]
}
plot_df = pd.DataFrame(columns=["threshold", "score", "build_strategy", "threshold_type", "score_type", "drg"])
for score_type in ['TPR']:
    for threshold_type, thresholds in threshold_type_dict.items():
        for threshold in thresholds:
            ...
            elif threshold_type == "Top-k% from all subgroups":
            ...
            for col in cur_cols:
                for index in cur_indexes:
                    temp_df = pd.DataFrame(data={
                        ...
                        "drg": [index],
                        "build_strategy": [col_name_dict[col]],
                        "score": [cur_res_df.loc[index, col]]
                    })
                    plot_df = pd.concat([plot_df, temp_df], axis=0, ignore_index=True)
plot_df.to_csv(f"/home/chenqinhai/code_eicu/my_lab/eicu_fairness_analysis/result/subgroup_top20_TPR_{race}_records_5-6.csv")


measure_avg_record = pd.DataFrame(columns=model_list)
measure_std_record = pd.DataFrame(columns=model_list)
measure_CV_record = pd.DataFrame(columns=model_list)
for threshold_id in threshold_used:
...
measure_avg_record.to_csv('/home/chenqinhai/code_eicu/my_lab/eicu_fairness_analysis/result/subgroup_top20_{}_fairness_avg_5-6.csv'.format(race))

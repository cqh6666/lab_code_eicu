...
from get_eicu_dataset import get_all_test_data_with_drg
...
def find_threshold_break_index(node_data_df: pd.DataFrame, threshold):
    """
    根据阈值找到对应的索引
    :param node_data_df:
    :param threshold:
    :return:
    """
    node_break_df = node_data_df[node_data_df[Constant.score_column] >= threshold]
    node_break_list = node_break_df.index.to_list()
    if len(node_break_list) == 0:
        return -1

    return node_break_list[-1]
...
class SubGroupDecisionTree:
    ...
    def build_tree(self, node: SubGroupNode, parent_index):
        ...
        # print info
        logger.warning(f"{node.depth}-[parent_index:{node.index}]: start selecting best split feature...")
        ...
        if len(node_list) == 0:
            node.set_leaf_node()
            logger.warning(f"{node.depth}-[parent_index:{node.index}]: no best split feature...")
            return
    ...
    def select_best_split_feature(self, node: SubGroupNode):
        ...
        logger.warning(f"{node.depth} - {node.index}: we have {len(remain_features)} remained features to select!")
        ...
        for feature in remain_features:
            ...
            logger.warning(
                f"{node.depth}-{node.index}: ({feature}) has {feature_unique_list} !")
            ...
            for feature_value in feature_unique_list:  # 一般是 0, 1
                ...
                if black_recall >= white_recall:
                    ...
                    # logger.warning(f"{node.depth}-{node.index}-({feature}:{feature_value}): black({black_recall})>=white({white_recall}), 不再继续分亚组...")
                else:
                    # 2.1.3 若 黑人召回率 < 白人召回率， 则需要进一步以最小损失进行分亚组。也就是需要对子节点进行修改阈值使得符合2.2。
                    # logger.warning(f"depth:{node.depth}-parent:{node.index}-[{feature}:{feature_value}]: black({black_recall})<white({white_recall}), 还得继续分亚组,开始调整...")
                    best_recall_result_dict, best_thresholds, split_loss = self.do_adjust_thresholds(temp_data_df)
                    # logger.warning(f"depth:{node.depth}-parent:{node.index}-[{feature}:{feature_value}]: 调整完成!")
    ...
    def do_adjust_thresholds(self, cur_data_df):
        ...
        white_data_df = cur_data_df[cur_data_df[Constant.white_race_column] == Constant.white_race].sort_values(
            by=Constant.score_column, ascending=False)
        black_data_df.reset_index(drop=True, inplace=True)
        white_data_df.reset_index(drop=True, inplace=True)

        # 计算黑人白人的阈值索引
        black_break_index = find_threshold_break_index(black_data_df, black_threshold)
        white_break_index = find_threshold_break_index(white_data_df, white_threshold)

        # 最多调整100次
        adjust_steps = int(white_break_index / 100) + 1

        # 计算黑人和白人的AKI总患者数量
        black_recall_nums, white_recall_nums = recall_result_dict['recall_nums']
        black_label_nums, white_label_nums = recall_result_dict['label_nums']

        # logger.debug(f"start to adjust...[{black_recall}, {white_recall}]")
        round_time = 0
        success_flag = True
        while black_recall < white_recall:
            round_time += 1
            black_break_index, black_break_add_nums, black_recall_add_nums, black_threshold = find_next_black_threshold(black_data_df, black_break_index, white_break_index, adjust_steps)
            white_break_index, white_recall_sub_nums, white_threshold = find_next_white_threshold(white_data_df, white_break_index, black_break_add_nums)
            black_recall_nums += black_recall_add_nums
            white_recall_nums -= white_recall_sub_nums
            black_recall = black_recall_nums / black_label_nums
            white_recall = white_recall_nums / white_label_nums
            if round_time > 105:
                success_flag = False
                logger.error("调整超过105次...")
                break

        # logger.debug(f"end to adjust...[{black_recall}, {white_recall}], cost {round_time} time!")
        # while black_recall < white_recall:
        #     logger.debug("start find black!")
        #     black_threshold, black_add_nums = find_next_black_threshold(black_data_df, black_threshold)
        #     logger.debug("end find black, and start white.")
        #     white_threshold = find_next_white_threshold(white_data_df, white_threshold, black_add_nums)
        #     logger.debug("end find white, and start cal recall.")
        #     recall_result_dict = cal_black_white_recall(cur_data_df, [black_threshold, white_threshold])
        #     logger.debug("end cal recall!")
        #     black_recall, white_recall = recall_result_dict['recall_rate']
        new_thresholds = [black_threshold, white_threshold]
        recall_result_dict = cal_black_white_recall(cur_data_df, new_thresholds)
        # 计算损失值
    ...
    def convert_format_to_dot_show(self):
        dot_begin_str = """
                digraph Tree {
        ...
        # labeldistance=2.5
        dot_link_str = """
                {} -> {} [minlen=3.5, labeldistance=5, splines=polyline, headlabel="{}"] ; 
                """
        dot_end_str = "}"
        ...
        explain_str = f'公平代价指标 : {self.splitLoss.name} \\n' \
                      f'初始阈值比例 = {risk_rate} \\n' \
                      f'决策树高度 = {self.max_depth} \\n' \
                      f'总共节点数 = {self.node_count + 1} \\n'
                      # f'初始代价损失 = {self.root_loss} \\n'
                      # f'最终代价损失 = {self.get_split_plus_loss()}'
        ...
        for node_index, node in self.node_tree.items():
            ...
            node_split_feature = None
            if not node.is_leaf:
                if node.sub_nodes is not None:
                    node_split_feature = node.sub_nodes[0].split_feature_name


            dot_info_str = f'SplitLoss = {node.split_loss} \\n' \
                           f'Nums(b/w) = ({node.all_nums[0]}/{node.all_nums[1]}) \\n' \
                           f'Recall(b/w) = ({round(node.recall_rate[0],2)}/{round(node.recall_rate[1],2)})'

            if not node.is_leaf:
                dot_info_str = dot_info_str + " \\n" + f'SplitFeature = {node_split_feature} \\n'

            # if node.split_feature_value % 2 == 1:
            ...
            if not node.is_leaf:
                if node.sub_nodes is not None:
                    ...
                    for tmp in temp_nodes:
                        all_dot_link_str += dot_link_str.format(node_index, tmp.index, f"{tmp.split_feature_name}={tmp.split_feature_value}")

        all_dot_str = dot_begin_str + all_dot_explain_str + all_dot_edge_str + all_dot_link_str + dot_end_str
        with open(f"/home/chenqinhai/code_eicu/my_lab/eicu_fairness_analysis/result/dot/sbdt_tree_rate{risk_rate}_type{split_type}_cross{cross_index}_show_v{version}.dot","w") as f:
            f.write(all_dot_str)
    ...
    def convert_format_to_dot(self):
        ...
        explain_str = f'risk_rate = {risk_rate} \\n' \
                      f'global_thresholds = {[round(x, 4) for x in self.global_thresholds]} \\n' \
        ...
        for node_index, node in self.node_tree.items():
            ...
            dot_info_str = f'LOSS({self.splitLoss.name}) = {node.split_loss} \\n' \
                           ...
                           f'thresholds = {[round(x, 4) for x in node.thresholds]} \\n' \
        ...
        all_dot_str = dot_begin_str + all_dot_explain_str + all_dot_edge_str + all_dot_link_str + dot_end_str
        with open(
            os.path.join(data_path, f"dot/sbdt_tree_rate{risk_rate}_type{split_type}_cross{cross_index}_v{version}.dot"), "w") as f:
            f.write(all_dot_str)
            logger.info("save as dot success!")

    def stop_to_split_condition(self, node, min_aki_nums=15, min_nums=50):
        """
    ...
    def predict(self, test_data_df: pd.DataFrame):
        ...
        for index in test_data_index:
            ...
            # 根据当前树结构找到对应的叶子节点的ID
            cur_leaf_index = self.get_leaf_index(temp_data)
            if cur_leaf_index == -1:
                leaf_index_df.loc[index, 'leaf_index'] = -1
                continue

            leaf_index_df.loc[index, 'leaf_index'] = cur_leaf_index
    ...
    def get_leaf_index(self, temp_data):
        ...
        while not cur_node.is_leaf:
            ...
            if not is_match:
                return -1
                # raise ValueError(f"cur_node:{cur_node} - test_data_index:{temp_data.index} - len_child_nodes:{len(cur_child_nodes)} - 匹配特征失败，中止匹配!")
...
def find_next_black_threshold(black_data_df: pd.DataFrame, black_break_index, white_break_index, adjust_steps):
    """
    找到下一个黑人的阈值
    :param adjust_steps:
    :param black_data_df:
    :param black_break_index:
    :param white_break_index:
    :return:
    """
    black_break_end_index = black_break_index + white_break_index
    black_data_nums = black_data_df.shape[0]
    # 限制范围
    if black_break_end_index >= black_data_nums:
        black_break_end_index = black_data_nums - 1
    elif black_break_end_index < 0:
        black_break_end_index = 0

    black_break_df = black_data_df.loc[black_break_index+1:black_break_end_index]

    # 判断AKI数量
    black_aki_df = black_break_df[black_break_df[Constant.label_column] == 1]
    if black_aki_df.shape[0] == 0:
        next_break_index = black_break_end_index
        next_break_threshold = black_data_df.loc[next_break_index, Constant.score_column]
        add_recall_nums = 0
    else:
        next_break_list = black_aki_df.index.to_list()
        if adjust_steps >= len(next_break_list):
            adjust_steps = len(next_break_list) - 1
        next_break_index = next_break_list[adjust_steps]  # 最多循环100次
        next_break_threshold = black_aki_df.loc[next_break_index, Constant.score_column]
        add_recall_nums = black_aki_df[black_aki_df[Constant.score_column] == next_break_threshold].shape[0]

    add_nums = next_break_index - black_break_index

    return next_break_index, add_nums, add_recall_nums, next_break_threshold


@DeprecationWarning
def find_next_black_threshold_old(black_data_df, black_threshold):
    """
    找到下一个阈值 阈值下降
...
def find_next_white_threshold(white_data_df, white_break_index, black_add_nums):
    """
    ...
    :return:
    """
    next_white_break_index = white_break_index - black_add_nums

    if next_white_break_index < 0:
        next_white_break_index = -1
        white_recall_sub_nums = 1
        white_threshold = white_data_df.loc[0, Constant.score_column] + 0.5
        return next_white_break_index, white_recall_sub_nums, white_threshold

    white_threshold = white_data_df.loc[next_white_break_index, Constant.score_column]
    white_break_df = white_data_df.loc[next_white_break_index:white_break_index]
    white_aki_df = white_break_df[white_break_df[Constant.label_column] == 1]
    white_recall_sub_nums = white_aki_df.shape[0]

    return next_white_break_index, white_recall_sub_nums, white_threshold


@DeprecationWarning
def find_next_white_threshold_old(white_data_df, white_threshold, black_add_nums):
    """
    根据黑人增加数目找到下一个白人阈值 阈值上升
    :param white_data_df:
...
def cross_predict():
    ...
    for index in all_range_list:
        ...
        train_data = get_all_test_data_with_drg(train_index)
        test_data = get_all_test_data_with_drg(test_index)
        ...
        sbdt.get_tree_info()
        sbdt.convert_format_to_dot_show()
    ...
    save_result_file = os.path.join(data_path, f"{myLoss.name}_test_cross_result_v{version}.csv")
...
if __name__ == '__main__':
    ...
    # split_type = 1
    # cross_index = 5
    logger.info(f"risk_rate:{risk_rate}, split_type:{split_type}, cross_index:{cross_index}...")
    """
    ...
    version = 16  EICU数据 种族 黑人和白人 max_depth=6
    """
    version = 16

    data_path = "/home/chenqinhai/code_eicu/my_lab/eicu_fairness_analysis/result"
    # eicu
    ...
    extra_cols = ["age"]
    my_cols = extra_cols + drg_cols + my_cols
    logger.info(f"select feature nums: {len(my_cols)}")
    pickle_file_name = os.path.join(data_path, f"/pickle/sbdt_pickle_rate{risk_rate}_type{split_type}_cross{cross_index}_v{version}.pkl")
    loss_dict = {
